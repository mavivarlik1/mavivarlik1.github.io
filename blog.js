...
export function initBlogModule(db, user, fsTools) {
    ...
    window.refreshBlogRender = function() {
        ...
        if (subHash) {
            ...
            if (post) {
                let mediaHtml = '';
                if (post.imageUrl) { mediaHtml += `<div style="margin:15px 0; text-align:center;"><img src="${post.imageUrl}" style="max-width:100%; max-height:400px; border-radius:12px; border:1px solid rgba(255,255,255,0.1); box-shadow:0 4px 20px rgba(0,0,0,0.4);" alt="Görsel"></div>`; }
                if (post.videoUrl) {
                ...
                const unsubComments = onSnapshot(commentsQ, (commentSnap) => {
                    const listEl = document.getElementById(`comments-list-${post.id}`); if (!listEl) return; listEl.innerHTML = '';
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            const data = docSnap.data(); let dateObj = new Date();
            ...
            localBlogCache.push({ id: docSnap.id, customId: customId, title: data.title, content: data.content, imageUrl: data.imageUrl || null, videoUrl: data.videoUrl || null, author: data.author, formattedDate: formattedDate });
        });
    ...
    window.createNewBlogPost = async function() {
        const titleInput = document.getElementById('blogTitleInput'); const contentInput = document.getElementById('blogContentInput');
        const imgInput = document.getElementById('blogImgInput'); const videoInput = document.getElementById('blogVideoInput');
        if (!titleInput || !contentInput) return;
        const title = titleInput.value.trim(); const content = contentInput.value.trim();
        const imageUrl = imgInput ? imgInput.value.trim() : ''; const videoUrl = videoInput ? videoInput.value.trim() : '';
        ...
        try {
            await addDoc(collection(db, "blogs"), { title: title, content: content, imageUrl: imageUrl, videoUrl: videoUrl, customId: customId, author: user.nick, uid: user.uid, createdAt: new Date() });
            titleInput.value = ''; contentInput.value = ''; if(imgInput) imgInput.value = ''; if(videoInput) videoInput.value = '';
            alert("Görsel ve video destekli makaleniz başarıyla yayına alındı."); window.location.hash = "#blog";
